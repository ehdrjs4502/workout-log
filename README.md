...
- **캘린더**로 날짜별 운동 내역
- **통계**로 종목·부위별 총 볼륨/총 횟수 추이 (꺾은선)
- 기록을 **텍스트로 내보내기** (AI에게 붙여넣어 평가받는 용도, 기간 직접 지정 가능)
...
src/
  app/
    ...
    exercises/            종목 관리
    settings/             알림·백업·저장소
    settings/export/      텍스트 내보내기
    manifest.ts           PWA manifest
  components/
    session/              로깅 화면 전용 컴포넌트
    stats/TrendChart.tsx  볼륨·횟수 2단 꺾은선 (인라인 SVG)
  ...
  lib/
    ...
    stats/history.ts      날짜·주 단위 집계 (통계 화면이 쓴다)
    stats/chartScale.ts   꺾은선 좌표 계산 (순수 — 스모크 테스트가 검증한다)
...
**기구(`Exercise.equipment`)는 표시·입력 전용이다.** 바벨/덤벨/머신/케이블/기타 다섯 가지가
종목 목록의 부제와 무게 ± 버튼의 증감폭(머신·케이블은 5kg)에만 쓰인다. `stats/volume.ts` 의
볼륨 공식은 이 값을 보지 않는다 — 기구를 나중에 고쳐도 지난 기록의 수치가 따라 움직이면 안 된다.
목록에 '맨몸' 이 없는 건 `usesBodyWeight` 가 이미 그 질문에 답하기 때문이다 (풀업 = 기타 + 맨몸).

**통계의 날짜는 `session.date` 가 정본이다.** 세트에는 `completedAt`(epoch ms) 밖에 없지만
그걸 날짜로 환산하면 자정을 넘긴 새벽 운동이 이틀로 쪼개진다. `listDatedSets()` 가 세션의 날짜를
세트에 붙여서 돌려주고, 통계 코드는 그 `date` 만 본다.

**통계의 두 선은 같은 구간 합계에서 뽑는다.** 총 볼륨과 총 횟수를 함께 그리는데, 한쪽만
"그날 최고 중량" 으로 잡으면 두 선이 반대로 움직인다 — 무게를 올린 날은 보통 횟수가 줄어서
60kg×10 → 80kg×5 로 나아간 게 횟수 선에서는 후퇴로 읽힌다. 최고 중량은 그래프가 아니라
요약 줄과 날짜별 기록표가 글자로 말한다.

**단위가 다른 두 값을 한 축에 올리지 않는다.** kg 과 회는 눈금을 어디에 맞추느냐가 전적으로
임의라서, 이중축으로 겹치면 데이터에 없는 상관관계를 그림이 지어낸다. 그래서 x축(날짜)만
공유하는 그래프 두 개를 위아래로 쌓는다.

**차트 라이브러리를 쓰지 않는다.** 필요한 게 꺾은선뿐이라 인라인 SVG 로 직접 그린다.
번들이 100KB 넘게 늘지 않고, `--color-chest` 같은 테마 토큰을 그대로 쓸 수 있다.
좌표 계산은 `stats/chartScale.ts` 에 순수 함수로 빼서 브라우저 없이도 검증한다.
...
- [ ] 통계 → 종목별에서 점을 탭하면 두 그래프의 같은 날짜가 함께 잡히고, 기간을 바꾸면 최신 날짜로 돌아옴
- [ ] 통계 → 부위별에서 부위를 바꾸면 두 그래프와 색이 따라 바뀜
- [ ] 설정 → 기록 텍스트 내보내기 → `직접` 으로 임의 기간 지정 (거꾸로 골라도 결과가 나옴)
- [ ] 종목 기구를 머신으로 바꾸면 무게 ± 버튼이 5kg 씩 움직임
